...
commandThrusterParticles(float amount, string direction) {
    llMessageLinked(LINK_ALL_CHILDREN, llFloor(amount), direction, id);
}

vectorThrusters(vector direction, vector rotationEuler, float time) {
    // direction is in global reference frame
    // So we need to rotate that back into the ship's rocket hrusts
    if (time == 0) {
        time = 1000;
    }
    vector localDeltaPos = direction / llGetRot();
    vector localDeltaRot = rotationEuler / time; // factor in thrust

    // Round off the vector to .01 in each axis.
    // More time is slower, so strength of thrust must be less    
    localDeltaPos = (localDeltaPos + <0.05, 0.05, 0.05>) * 10.0 / time;
    ...
    if (localDeltaPos.x > 0) {
        commandThrusterParticles(localDeltaPos.x, "fwd");
    } else if (localDeltaPos.x < 0) {
        commandThrusterParticles(-localDeltaPos.x, "back");
    }
    if (localDeltaPos.y > 0) {
        commandThrusterParticles(localDeltaPos.y, "LEFT");
    } else if (localDeltaPos.y < 0) {
        commandThrusterParticles(-localDeltaPos.y, "RIGHT");
    }
    if (localDeltaPos.z > 0) {
        commandThrusterParticles(localDeltaPos.z, "up");
    } else if (localDeltaPos.z < 0) {
        commandThrusterParticles(-localDeltaPos.z, "down");
    }
    if (localDeltaRot.z > 0) {
        commandThrusterParticles(localDeltaRot.z, "left");
    } else if (gFrameDeltaEuler.z > 0) {
        commandThrusterParticles(-localDeltaRot.z, "right");
    }
    llSleep(0.1);
}
...
flyAndRotateToNextPosition() {
    ...
    rotation isRot = llGetRot();
    vector isEuler = llRot2Euler(isRot);
    vector targetEuler = llRot2Euler(getWaypointRot(gFrame));
        
    if (gStep == 0) {
        float frameDuration = getWaypointTime(gFrame);
        vectorThrusters(-gFrameDetaPos, -gFrameDeltaEuler, frameDuration);
        gSteps = frameDuration / gStepInterval;
        if (gSteps == 0) {
            llSay(0,"flyAndRotateToNextPosition ERROR: at gFrame "+(string)gFrame+" gSteps == 0");
            gFrame = gNumKeyFrames;
            return;
        }
        ...
        gStepDeltaEuler = gFrameDeltaEuler / gSteps;
    }
    ...
    control(key Pilot, integer levels, integer edges)
    {
        ...
        if (thrustDirection != "")
        {
            ...
            if (llGetTime() > (gLastMessage + 0.5)) {
                commandThrusterParticles(TARGET_INCREMENT, thrustDirection);
...
state AutomatedFlight
{
    ...
    timer()
    {
        if (gFrame < gNumKeyFrames) {
            ...
            // finished
            // Final cancel thrust
            vectorThrusters(-gFrameDetaPos, -gFrameDeltaEuler, getWaypointTime(gNumKeyFrames-1));
